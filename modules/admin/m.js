...
widgets.setting = function(input) {
  ...
  this.toHTML = function() {
    ...
    for(var k in data) {
        if (k != 'id' && k != '_id') {
            ...
            switch (type) {
                ...
                break;
            	case 'boolean':
                form[k] = fields.boolean(_.extend(settings,{value : v, label : readable_name}));
                break;
            	case 'number':
                form[k] = fields.number(_.extend(settings,{value : v, label : readable_name}));
                break;
            	case 'email':
    ...
    var form_html = reg_form.toHTML(function (name, object) { return bootstrap_field(name, object); });
